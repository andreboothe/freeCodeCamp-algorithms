...
function myReplace(str, before, after) {
    var arrB = before.split(''); 
    var arrA = after.split(''); 
    
    if(before[0] == before[0].toUpperCase()){
      arrA.splice(0,1, arrA[0].toUpperCase());
    }else{ arrA.splice(0,1, arrA[0].toLowerCase());}
    
    return str.replace(arrB.join('') ,arrA.join(''));
  }